...
//        clazzSet.forEach(System.out::println);
    ...
    public static Set<String> findPackageClass(String scanPackages) {
        ...
        for (String basePackage : packages) {
            ...
            //packageSearchPath=classpath*:com/std/smartcampus/service/**/*.class
            ...
            try {
                //实际是此类 PathMatchingResourcePatternResolver
        ...
        for (String pInArr : packages.toArray(new String[packages.size()])) {
            ...
            while (packageIte.hasNext()) {
                ...
                if (pInArr.startsWith(pack + ".")) {
                    //如果待加入的路径是已经加入的package的子集，不加入
                    needAdd = false;
                } else if (pack.startsWith(pInArr + ".")) {
                    //如果待加入的路径是已经加入的pack的父集，删除已加入的package
                    packageIte.remove();
                }
            }
            if (needAdd) {
